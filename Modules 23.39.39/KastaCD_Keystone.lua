-- KastaCD_Keystone.lua - adds "Ready Check" and "Pull Timer" buttons to
-- the Mythic+ keystone frame (ChallengesKeystoneFrame), and best-effort
-- auto-inserts the player's keystone as soon as it opens.
-- Auto-insert uses UseContainerItem() on the keystone while the frame is
-- open - the same everyday "right-click to use" action, rather than
-- manually dragging the item onto the slot (taint-risky). Wrapped in
-- pcall and gated by its own toggle.
-- Detection matches the shared "Keystone" name substring, not one
-- hardcoded item ID, since each dungeon's keystone has its own item ID.
...
-- Global - also used by KastaCD_KeyAnnouncer.lua.
...
-- TOOLTIP is the highest strata, guaranteeing the frame draws above action bars.
...
-- Blizzard_ChallengesUI may already be loaded by the time this file runs.
if _G.ChallengesKeystoneFrame then
    watcher:UnregisterEvent("ADDON_LOADED")
    HookKeystoneFrame(_G.ChallengesKeystoneFrame)
end

-- /kcdks - prints Keystone/Challenge-named frames and matching bag items.
